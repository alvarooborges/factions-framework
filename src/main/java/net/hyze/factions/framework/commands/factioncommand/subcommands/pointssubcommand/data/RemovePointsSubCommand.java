...
public class RemovePointsSubCommand extends CustomCommand implements GroupCommandRestrictable {

    public RemovePointsSubCommand() {
        super("remove", CommandRestriction.IN_GAME);

        registerArgument(new Argument("TAG", "Tag da facção.", true));
        registerArgument(new Argument("valor", "Pontos que deseja remover.", true));
    }
    ...
    public void onCommand(CommandSender sender, User user, String[] args) {
        ...
        if (faction.getPoints() <= points) {
            points = 0;
        } else {
            points = faction.getPoints() - points;
        }
